...
\begin{frame}\frametitle{Overview}
	...
	\begin{itemize}
		\item[{\tiny [{\bf Origins}]}] Most theories emphasize how important fiscal capacities are for state-building. {\color{red}However}, most theories don't explain where these capacities come from.

		\item[{\tiny [{\bf Measurement}]}] Most theories provide {\bf historical} explanations for state-building, and {\color{red}yet}, these theories lack of {\bf historical} measurements able to capture levels of state formation over time.\pause
...
\begin{enumerate}
	\item Higher levels of sectoral/elite competition promoted the implementation of the income tax.\pause
	\item The income tax fostered higher levels of state capacities overtime.\pause
	\item Earthquake death-tolls are good proxies to measure of state capacities.\pause The {\bf capacity} of the state to {\bf enforce} quake-sensitive {\bf building codes} throughout the territory, is a {\bf {\color{red}reflection}} of its {\bf overall} state-capacities.
\end{enumerate}
...
\begin{itemize}
	\item[] There exists both a {\bf scientific} and a {\bf popular} consensus on that {\bf building codes} \emph{do} reduce death tolls. {\bf Death tolls} are mostly a function of state-capacities.
\end{itemize}
\vspace{-0.7cm}
	\begin{columns}
...
\miniframesoff
\subsection{Dual Economy}
\begin{frame}\frametitle{Dual Economy and Sectoral Conflicts}
...
\miniframeson
\begin{frame}\frametitle{Case Study Findings}
	\begin{itemize}
		...
		\begin{itemize}
			\item Industrialists preferred to impose an income tax on themselves.\pause\; {\bf Given their higher dependence on imports (capital), imposing an import tax would have made them worse off}.\pause
		\end{itemize}

	\item[{\tiny{\bf Other paper}}] In the early 20th century in L.A., the {\bf emergence of industrial challenger elites} fostered the {\bf {\color{red}earlier}} implementation of the income tax. \hyperlink{duration_models}{\beamerbutton{Models}}.
...
\begin{frame}[plain,c]
	\begin{center}
	\vspace{5mm}
	\Huge Did the implementation of the income tax increase state capacities over time? \\ If so, how those can be measured?
	\end{center}
\end{frame}
