...
\author{H\'ector Bahamonde $\bullet$ Assistant Professor $\bullet$ O$'$Higgins University (Chile)}
\date{\today}
...
\begin{frame}\frametitle{Theory}
  \begin{itemize}
    \item Others have explored the positive influence of direct taxation on state development.
    \item Unfortunately:
      \begin{enumerate}
        \item Most accounts focus on European cases.\pause
        \item The role of structural economic transformations has been overlooked.
      \end{enumerate}
  \end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Measurement}
	\begin{itemize}
    \item Most state formation theories put their emphasis on pre-colonial, early, or independent Latin America.\pause
    \item Unfortunately, these theories lack of a measurement able to capture levels of state capacity \emph{overtime}.
	\end{itemize}
\end{frame}


\miniframesoff
\begin{frame}\frametitle{Taxation and State Capacity in Chile}

{\bf Convince you}:

\begin{enumerate}
	\item Higher levels of sectoral competition promoted the implementation of the income tax in Latin America.\pause
	\item The income tax fostered overtime higher levels of state consolidation in Chile.\pause
	\item Earthquake death-tolls are good-enough proxies to measure overtime state capacity in Chile.
\end{enumerate}
...
\begin{itemize}
	\item[] The {\bf capacity} of the state to {\bf enforce} quake-sensitive {\bf building codes} throughout the territory, is a {\bf {\color{red}reflection}} of its {\bf overall} state-capacity.
\end{itemize}
...
\begin{itemize}
	\item[] Earthquakes are {\bf exogenous} to regime type and levels of political/economic development.
\end{itemize}
...
%		\hspace{-7mm}\frame{\includegraphics[scale=0.8]{/Users/hectorbahamonde/RU/Dissertation/Papers/Earthquake_Paper/Resources/la_moneda}}
%		\end{figure}
%	
%	\end{columns}
%
...
\begin{frame}\frametitle{Dual Political Economy and Taxation}
    ...
    \item \emph{``Lewis model:''} Industrialists and agriculturalists are in permanent {\bf {\color{red} economic conflict}}.\pause\; I {\bf extrapolate this conflict to politics}:\pause

            \begin{itemize}

                \item[] There are different {\bf sectoral preferences towards income taxation}, and consequently, {\bf state centralization}\pause:
              ...
              \begin{enumerate}
                \item[{\color{red}[Agr]}] Since {\color{red}land fixity} increases the risk of the landed elite's main asset, landowners systematically {\color{red}resist} taxation.\pause
...
% why income tax
\miniframeson
\begin{frame}\frametitle{The Spillover Effects of Taxation on State Capacity}

	\begin{itemize}
		\item \emph{``Fiscal sociology:''} Income taxation offers a theory of state formation.\pause

		\item Monitoring private incomes, and converting them into public property, \emph{fosters} state development:\pause

			\begin{itemize}
				\item  ``{\bf Learning-By-Doing}'': If states want to {\color{red}{\bf tax}} its population, they \emph{will} develop---{\color{red}{\bf endogenously}}---the {\bf {\color{red}{\bf capacity}}} to do it.\pause
        \item ``{\bf Spillover Effects}'': Income taxation generated positive {\bf {\color{red}spillover effects}} for state-making, rising {\bf {\color{red}economies of scale}} of the {\bf {\color{red}operational efficiencies}} of the bureaucracy:\pause
                \begin{itemize}
                  \item Income taxation represented the {\bf first layer of state-complexity}.
                  \item {\bf Additional layers} (bureaucracies) cheaper at the margin.
                \end{itemize}
			\end{itemize}
...
%	\begin{center}
%	\vspace{5mm}
%	\Huge Did the implementation of the income tax in Chile increase state capacity over time? \\ \pause\;If so, how can those be measured?
%	\end{center}
%\end{frame}
...
%\begin{frame}[plain,c]%\frametitle{Research Question}
%\begin{center}
%	\Huge{{\bf State capacity increase overtime upon the implementation of the income tax}}
%\end{center}
%\end{frame}
...
\begin{frame}\frametitle{Argument (I)}
	\begin{itemize}
		...
		\item[] The emergence of a {\color{red}strong industrial class}, accelerated the {\color{red}implementation of the income tax}. {\color{white}In turn, income taxation fostered {\color{white}state-capacity} overtime.}
...
\begin{frame}\frametitle{Argument (II)}
  \begin{itemize}
    ...
    \item[] The emergence of a {\color{black}strong industrial class}, accelerated the implementation of the {\color{red}income tax}. {\color{black}In turn, income taxation fostered {\color{red}state-capacity} overtime.}
...
%  \begin{itemize}
%    
%    \item[] The sectoral threats had to do with the {\bf emergence} of an \emph{industrialists} class that were more {\color{red}sympathetic} to the idea of implementing an income tax law.
%
%
%    \begin{figure}[H]
...
%
%\miniframeson
%\begin{frame}\frametitle{{\large Chilean Shadow Case}}
%	\begin{itemize}
...
\begin{frame}\frametitle{The Theory Should Pass The Following Tests}
	
	\begin{enumerate}
		\item The income tax law should be {\bf implemented {\color{red}earlier}} under scenarios of {\color{red}\emph{high} sectoral contestation}.\\{\footnotesize [\emph{rapid} industrial expansion]} \hyperlink{cox}{\beamerbutton{Data/Equation}}\pause

		\item Implementation of the {\color{red}income tax} should foster {\color{red}\emph{higher} state-capacity}.\\{\footnotesize [\emph{lower} death tolls overtime]} \hyperlink{temp:dist}{\beamerbutton{Temp. Distribution of EQs}}
...
\subsection{{\bf Test 1}: Duration Models. Income Taxation and Sectoral Contestation}

    \begin{frame}[label=cox]\frametitle{Early and Late Income-Tax Implementers}


        \begin{enumerate}
          \item[] {\bf Cox Model}. Sectoral outputs (\href{http://moxlad-staging.herokuapp.com/home/en?}{MOxLAD dataset}) to investigate the sectoral contribution to the {\color{red}timing} of the implementation of the income tax in 15 Latin American countries.\pause
...
\begin{frame}\frametitle{Early and Late Income-Tax Implementers}
        ...
        \begin{enumerate}
          \item[] {\bf Cox Model}. Sectoral outputs (\href{http://moxlad-staging.herokuapp.com/home/en?}{MOxLAD dataset}) to investigate the sectoral contribution to the {\color{red}timing}  of the implementation of the income tax in 15 Latin American countries.
...
\end{frame}    
        ...
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/chile_income_tax_plot.pdf}
  \end{frame} 
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/ecuador_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/nicaragua_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/venezuela_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/peru_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/colombia_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/guatemala_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/argentina_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/mexico_income_tax_plot.pdf}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/brazil_income_tax_plot.pdf}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/bolivia_income_tax_plot.pdf}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/costa_rica_income_tax_plot.pdf}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/honduras_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/paraguay_income_tax_plot}
  \end{frame}
        
        \miniframesoff
        \begin{frame}[plain]\frametitle{The Timing of the Implementation of the Income Tax}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.4]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/uruguay_income_tax_plot.pdf}
  \end{frame}
...
\miniframesoff
\begin{frame}[plain]\frametitle{{\large Implementing the Income Tax: Simulated Hazards}}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.8]{/Users/hectorbahamonde/RU/Dissertation/Papers/IncomeTaxAdoption/figure/simulation:plots-1.pdf}
\end{frame}



\subsection{{\bf Test 2}: Count Model. Spillover Effects of Income Taxation}

\miniframesoff
\begin{frame}
\centering\huge Did the income tax foster state consolidation over time?
\end{frame}

\miniframesoff
\begin{frame}[plain]\frametitle{{\large Geographic Distribution of EQs }}
 %\vspace{-0.5cm}
  \centering\includegraphics[scale=0.27]{/Users/hectorbahamonde/RU/Dissertation/Papers/Earthquake_Paper/figure/earthquake:map:plot:chile-1.pdf}
\end{frame}
...
\begin{frame}\frametitle{State Capacity Overtime in Chile}

{\bf {\footnotesize Bayesian Poisson model. \hyperlink{jags_code}{\beamerbutton{Jags Code}}}}
...
\begin{split}
i_{1,...I} & \; \text{and} \; \text{I}=103\; \text{events};\\
k_{1,...K} & \; \text{and} \; \text{K}=3 \; \text{sectors};\\
%t_{1,...T} & \; \text{and} \; \text{T}=64 \; \text{years}.\\
\end{split}
\end{equation*}
...
\miniframeson
\subsection{Results: State Capacity Overtime}


\begin{frame}[plain]\frametitle{{\large Income Taxation Increases State Development Overtime {\scriptsize (Chile)}}}
 %\hspace{-1.16cm}
  \centering\includegraphics[scale=1]{/Users/hectorbahamonde/RU/Dissertation/Papers/Earthquake_Paper/figure/income:tax:model:plot:run-1.pdf}
      \hyperlink{table_tax}{\beamerbutton{Regression Table}}
...
\begin{frame}\frametitle{Summary}
	\begin{itemize}
		 \item The {\bf emergence of a strong industrial sector}, accelerated the implementation of the \emph{income tax} in 15 countries in Latin America.

		 \item {\bf Income taxation} increased overtime levels of \emph{state capacity} in Chile.
     %\item Via spillover effects, the \emph{act} of collecting taxes, improved other state activities (providing public goods and enforcing building codes).
	\end{itemize} 

%Why care?
  
 % \begin{enumerate}
  %   \item {\bf A novel earthquake dataset} was used to measure state capacity overtime. \pause Under reasonable assumptions, the capacity of a state to enforce building codes, is a reflection of its overall state capacity. \pause

   %  \item \emph{Unlike} other theories, my paper stressed the {\bf domestic factors} at play {\scriptsize(\emph{sectoral contestation})}, relaxing the international conflict---\emph{Tillian}---hypothesis.
  %\end{enumerate} 
...
\begin{frame}\frametitle{Next Steps}
	\begin{itemize}
		 \item Have three separate papers:
		 	\begin{enumerate}
		 		\item Timing of the income tax.
		 		\item Spillover effects of taxation on state consolidation.
		 		\item Measurement paper (earthquakes).
		 	\end{enumerate}
	\end{itemize} 
\end{frame}

\miniframesoff
\begin{frame}\frametitle{{\large Measurement Paper: Earthquakes as Repeated Natural Experiments}}
 %\hspace{-1.16cm}
  \centering\includegraphics[scale=0.355]{/Users/hectorbahamonde/RU/Dissertation/Papers/Earthquake_Paper/chile_peru_map.pdf}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\miniframesoff
\begin{frame}[label=temp:dist,plain]\frametitle{{\large Temporal Distribution of EQs}}
 %\hspace{-1.16cm}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[label=cox_simulations,plain]\frametitle{{\large Sectoral Contestation, and the Timing of the Income Tax Law {\scriptsize (9 Lat Am Count)}}}
 %\hspace{-1.16cm}
