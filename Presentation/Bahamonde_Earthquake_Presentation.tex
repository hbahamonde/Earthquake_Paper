...
\begin{frame}\frametitle{Overview}
	...
	\begin{itemize}
		\item[{\tiny [{\bf Origins}]}] Most theories emphasize how important fiscal development is for state consolidation. {\color{red}However}, the origins of fiscal development are less clear.

		\item[{\tiny [{\bf Measurement}]}] Most theories provide {\bf historical} explanations for state consolidation, {\color{red}yet}, they lack of a {\bf historical} measurement capable of capturing levels of state capacities overtime.\pause

		\item I find that these gaps represent important {\bf theoretical} and {\bf empirical}  {\color{red}\bf deficits}.
	\end{itemize}
...
\begin{frame}\frametitle{Dual Political Economy and Taxation}
	...
	\begin{itemize}
		\item \emph{``Lewis model:''} Industrialists and agriculturalists are in permanent {\bf {\color{red}conflict}}.\pause\; I {\bf {\color{red}extrapolate this conflict to politics}.\pause\; Particularly, to their respective {\color{red}sectoral preferences towards income taxation}}, and consequently, {\bf state centralization}.\pause
...
\begin{frame}\frametitle{Taxation and State Capacities}
	...
	\begin{itemize}
		\item \emph{``Fiscal sociology:''} Income taxation offers a mechanism to explain state development.\pause

		\item Monitoring private incomes, and converting them into {\color{red}public property}, \emph{fostered} state development:\pause

			\begin{enumerate}
				%\item {\color{red}\emph{Indirect} taxes are {\bf easier} to collect}: ex., collect them at ports. \hyperlink{why_not_indirect_taxes}{\beamerbutton{appendix}}
		...
		\item[] {\scriptsize{\color{black!30!green}In simple, the same bureaucracies that were sent to collect taxes, used the acquired knowledge to perform other state tasks}}.
	\end{itemize}
...
\begin{frame}\frametitle{Argument}
	\begin{itemize}
		...
		\item[] Higher levels of subnational and national {\bf {\color{red}sectoral contestation}} fostered state-capacities overtime.
...
\subsection{Meaning of Sectoral Contestation}
\miniframeson
\begin{frame}\frametitle{Conceptualizing Contestation}
	\begin{itemize}
		
		\item[] {\bf Industrial} expansion in the {\bf{\color{red}regions}}, challenged the {\color{red}national}/{\color{red}hegemonic} {\bf agricultural} political economy.
...
\begin{frame}\frametitle{{\large Chilean Case Study}}
	\begin{itemize}
		\item Industrial expansion posed credible threats to the landed elites. \hyperlink{credible_threats}{\beamerbutton{Evidence}}\pause

		\item Agreements were required to avoid more sectoral conflicts\pause: {\bf {\color{red}income tax}}. \hyperlink{duration_models}{\beamerbutton{Duration Models}}\pause

		\item Industrial elites agreed to comply with the central level and pay the {\bf income tax} in exchange of public goods delivered at the local level (bridges, ports, etc.).
...
\begin{frame}\frametitle{The Theory Should Pass Two Tests}
	...
	\begin{enumerate}
		\item The state should have higher capacities (i.e. \emph{lower death tolls}) when it's subnationally contested.
...
\begin{frame}\frametitle{Analyses}
	...
	\begin{itemize}
		\item[Data] Subnational and national data on Chile (1907 to 2012).
				
				\begin{itemize}
					\item[{\tiny National}] Sectoral outputs to measure sectoral competition. \hyperlink{proportion_variable}{\beamerbutton{plot}}
					
					\item[{\tiny Subnational}] Complemented with archival data (historical newspapers and censuses from 1907 onwards), the NOAA Earthquake database\pause:

						\begin{enumerate}
							\item Local population (to weight death tolls).
							\item Local main economic sector.
						\end{enumerate}
				
				\end{itemize} 

		\pause \item[Model] Bayesian Hierarchical Poisson model with year fixed-effects, to account for the count of deaths. \hyperlink{jags_code}{\beamerbutton{Jags Code}}
	\end{itemize}
...
\subsection{Model 1}
\miniframeson
\begin{frame}\frametitle{{\large Subnational Contestation and State Development}}
...
log(\lambda_{i})  &= 
\;\mu + 
\beta_{1_j}\text{Proportion$\frac{\text{{\tiny Agr}}}{\text{{\tiny Ind}}}$}_{i} + 
...
j_{1,...J} & \; \text{where} \; \text{J}=3\;\text{sectors @ subnational level (agr, ind, mixed)}\\
...
log(\lambda_{i})  &= 
\;\mu + 
\beta_{1_j}\text{{\color{red}Proportion}$\frac{\text{{\tiny {\color{red}Agr}}}}{\text{{\tiny {\color{red}Ind}}}}$}_{i} + 
\beta_{2_j}\text{Magnitude}^2_{i} +
\beta_{3}\text{Latitude}_{i} + \\
& \beta_{4}\text{Longitude}_{i} +
\beta_{5}\text{Population}_{i} +
...
log(\lambda_{i})  &= 
\;\mu + 
{\color{red}\beta_{1_j}}\text{Proportion$\frac{\text{{\tiny Agr}}}{\text{{\tiny Ind}}}$}_{i} + 
\beta_{2_j}\text{Magnitude}^2_{i} +
\beta_{3}\text{Latitude}_{i} + \\
& \beta_{4}\text{Longitude}_{i} +
\beta_{5}\text{Population}_{i} +
...
\begin{split}
i_{1,...I} & \; \text{where} \; \text{I}=91\;\text{earthquakes}\\
j_{1,...\text{{\color{red}J}}} & \; \text{where} \; \text{{\color{red}J}}=3\;\text{{\color{red}sectors @ subnational level (agr, ind, mixed)}}\\
t_{1,...T} & \; \text{where} \; \text{T}=59\;\text{years.}
\end{split}
\end{equation*}
\normalsize
...
\begin{frame}\frametitle{{\large Subnational Contestation Increases State Capacities}}
 %\hspace{-1.16cm}
...
\subsection{Model 2}
\begin{frame}\frametitle{{\large Income Taxation and State Development Overtime}}
...
\begin{frame}\frametitle{{\large Income Taxation Increase State Development Overtime}}
 %\hspace{-1.16cm}
...
\begin{frame}\frametitle{Summary}
	\begin{enumerate}
		 \item {\bf Sectoral contestation}, between a strong national agriculture, and strong regional industrial clusters, helped to foster state consolidation.

		 \item {\bf Income taxation} increased state capacities overtime.

		 \item {\bf Limitations}: this rationale only applies for cases where there are earthquakes. 
	\end{enumerate}

What's ahead:

	\begin{itemize}
		 \item More data should be collected, and the mechanism should be examined in other countries of the region.
	\end{itemize}
...
\begin{frame}[label=duration_models]\frametitle{Duration Models}
Duration models
\end{frame}
