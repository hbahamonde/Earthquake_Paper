...
As argued before, a potential issue might be that sectors self-select into more/less earthquake-prone geographical locations. For instance, it might be argued that agricultural areas, being mostly rural, might have lower constructions, and less populated areas, with potentially less earthquake death-tolls. Industrialists, in turn, being a more urban-oriented sector, might have both highly populated municipalities, and higher constructions (edifices), potentially showing higher default death-tolls. 


% sectoral self-selection
To rule-out this possibility, $\beta_{7,k}$---in \autoref{model:2}---has a hierarchical structure which allows having three different intercepts, one per every $k$ sector. In other words, this parameter shows whether earthquake magnitudes affect earthquake death-tolls in agricultural, industrial or mixed municipalities, in different ways. \autoref{income:tax:model:regression:table:run} strongly suggests that industrial areas do not have higher earthquake death-tolls when compared to agricultural areas. The posterior predicted means for the former are \Sexpr{round(reg.results.table.tax$Mean[row.names(reg.results.table.tax)=="Sector[Industry]"],0)} casualties, and for the latter, \Sexpr{round(reg.results.table.tax$Mean[row.names(reg.results.table.tax)=="Sector[Agriculture]"],0)} deaths. Similarly, mixed municipalities have a predicted death-toll of \Sexpr{round(reg.results.table.tax$Mean[row.names(reg.results.table.tax)=="Sector[Mixed]"],0)}.


\todo{Diagnostics here: traplot, etc.}
...
To have a more substantive grasp, and using simulation methods, \autoref{fig:simulation:plots} shows the hazard rates of the Cox model (model 1 in \autoref{results:table:cox}, or \label{cox:eq}). The simulation considers both the maximum and the minimum output rate in the entire dataset. THe figure shows that it is not \emph{overall} growth that accelerates the implementation of the income tax, but \emph{industrial} growth. In fact, agricultural expansion \emph{delays} the implementation of the law, harming state development in the long run. Since industrialists preferred the income tax, these analyses suggest that industrial expansion is systematically associated with the implementation of the income tax, not only in Chile, but in other Latin American polities too. These results support the idea that income tax is directly related with industrial expansion rather than economic development, but also support the idea of the sectoral \emph{conflict} behind its implementation. Since each sector had their own preferences toward income taxation, the emergence of the industrial sector facilitated state development by implementing the income tax, while agricultural expansion not only delayed the income tax, but also compromised state formation.
...
% geographical self-selection
% A potential issue might be whether certain sectors self-select into less earthquake-prone geographical locations. For instance, it might be argued that agricultural areas, being mostly rural, might have lower constructions, while industrial areas, being more urban, might have both highly populated municipalities and higher constructions (edifices), potentially showing higher default death-tolls. To rule-out this possibility, a hierarchical variant of \autoref{model:2} was estimated. Using the same set of control variables, \autoref{model:1} considers whether earthquake magnitudes affect the death-toll of agricultural, industrial or mixed municipalities in different ways. More formally, \autoref{model:1} was fitted: 
