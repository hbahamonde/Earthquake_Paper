...
\autoref{income:tax:model:regression:table:run} shows the estimated results. Methodologically, however, the coefficient of the interaction term remains uninterpretable in practice, making regression tables useless from a substantive standpoint \parencite[74]{Brambor2006}. These problems get more complex when it comes to generalized models, as a number of challenges arise. In an important paper, \textcite{Ai2003} explain that the interaction effect could be nonzero, even when the coefficient says it is zero; the statistical significance of the interaction effect cannot be tested with a simple t-test on the coefficient of the interaction term; the interaction effect is conditional on the independent variables; and that the interaction effect may have different signs for different values of covariates. In addition to all these challenges, and given that cross-partial derivatives are not advisable either, simulation methods are required \parencite[]{WilliamJ.Wales2013,King2000}. This procedure samples via simulation from the point estimates, generating a new and larger distribution. That is, taking the single estimated parameters (i.e. the regression coefficients), I constructed a distribution of estimated values for each coefficient. Relying on the central limit theorem, with enough sampling draws, the new simulated distribution is a transformation that approximates with a great degree of precision the (uninterpretable) coefficients. Subsequently, means and uncertainty measures can be constructed for each of these distributions. 

% methodological solution
Fortunately, the Bayesian framework has embedded these procedures into it, providing a systematic, and yet intuitive, methodology to solve these challenges. In particular, for every coefficient, I estimated \Sexpr{n.chains.tax} chains, with \Sexpr{n.iter.tax} iterations per chain. And considering the Monte Carlo Markov Chain properties, I discarded the first \Sexpr{n.burnin.tax} iterations of every chain. Following \parencite[]{Brambor2006}, \autoref{fig:income:tax:model:plot:run} shows the conditional effect of earthquake magnitudes on implementing the income tax.\footnote{Based on \autoref{model:2}, the simulated matrix was constructed following the routine described above, iterating over the next generalized equation: $\beta_{1}+\beta_{3}\times\text{Income Tax}_{i}$.} 

% substantively
Substantively, \autoref{fig:income:tax:model:plot:run} shows that the death-toll systematically \emph{decreases}---i.e. state capacity systematically \emph{increases}---overtime \emph{once the income tax law is implemented}. Particularly, by implementing the income tax, the baseline propensity of earthquake of increasing the death-toll, \emph{decreases} from an estimated overtime average of $\Sexpr{death.toll.before.tax}$ to an estimated overtime average of $\Sexpr{death.toll.after.tax}$. Importantly, the fixed-effects approach makes these results robust to considerations of progressive economic development. By absorbing yearly sources of variation, \autoref{model:2} is capable of controlling for unobserved factors that might have an impact on the dependent variable (\autoref{fig:year:fixed:effects:plot:plot} shows the estimated year fixed-effects).


% HERE
\todo{Diagnostics here}
