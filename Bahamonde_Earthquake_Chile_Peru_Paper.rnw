...
\end{equation}
where $\alpha$ are the country fixed effects for all countries $i$.} One important advantage of this strategy is the ability to account for country-specific effects. For example, fiscal expansion could be a function of country-specific prior state-building capacities.\footnote{I thank Matthias vom Hau for this suggestion.} A number of scholars rightly argue that post-colonial state-capacities are in part a function of pre-colonial state-capacities (\cite[1416]{Wimmer2016}, \cite{Mahoney:2010aa}, \cite[1426]{Lange2006}). Fixed-effects should be able to account for these and other unobserved or hard-to-measure covariates, which if left unaccounted for, would introduce omitted variable biases \parencite{Angrist:2008fk}. 
